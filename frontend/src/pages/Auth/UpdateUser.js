	...
	// state variables
		...
		const submitThis=(e)=>{
			...
			if (flag){
				...
				axios.post("/login/updateuser/",payload)
				.then(result =>{
                        if( result.data.code === "user error"){ // failed token 
        ...
        useEffect(()=>{
            axios.post("/login/get_me/",{"token":Token})
            .then(result =>{ 
		...
		useEffect(()=>{
		    if (!(Token === LoggedAs) && (!Token || !["student","staff","admin"].includes(LoggedAs))){
			  ...
			  <div className='divPad-big'/>
                ...
                <p>Enter the new values if you want to change them</p>
				  <form>
					...
					<input type='text' placeholder='Enter new first name' name="firstname" id="firstname" onChange={handleChange}/>
					<label>Last Name : {oldData.lastname} </label>
					<input type='text' placeholder='Enter the new last name' name="lastname" id="lastname" onChange={handleChange}/>
					<label>User-Id: {oldData.UserId}</label>
					<input type='text' placeholder='Enter the new user id' name= "UserId" id="UserId" onChange={handleChange}/>
					<label>Email: {oldData.email}</label>
					<input type='email' placeholder='Enter the new email' name="email" id="email" onChange={handleChange}/>
					<label>Contact: {oldData.contact}</label>
					<input type='number' placeholder='-' name="contact" id="contact" onChange={handleChange}/>
