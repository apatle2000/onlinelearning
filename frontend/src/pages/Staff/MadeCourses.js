...
function MadeCourses(){
  ...
  //run the first time
  useEffect(()=>{
      axios.post("/courses/getstaffcourses/",{"token":Token})
            .then(result =>{
                console.log(result); 
      ...
      if(e.length>0){
          return (
              ...
              {e.map( item =>(
                      ...
                      <div onClick={()=>{navigate("/coursemanager",{state:{...item}})}}>
  ...
  return (
      <>
          <div>
              <label>This page contains all the Courses created by you</label>
          </div>
