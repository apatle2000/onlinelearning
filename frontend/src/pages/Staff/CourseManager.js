...
function CourseManager(){
    ...
    const loadmoduletests = () =>{
      toggle = true;
      axios.post("/courses/getcoursetests/",{"token":Token,"course_id":location.state.course_id})
        .then(result =>{
            console.log(result); 
            if(result.data.code === 200){
              UpdateContent(result.data.values); 
        ...
        return (
            ...
            {v.map( item =>(
                    <Grid item xs={2} sm={4} md={4} key={item["test_id"]}>
                        <div onClick={()=>{console.log(item["test_id"],item)}}>
          ...
          return (
              ...
              {v.map( item =>(
                      <Grid item xs={2} sm={4} md={4} key={item["course_id"]}>
                          <div onClick={()=>{console.log(item["course_id"],item)}}>
    ...
    return (
      <>
          <Container className='pagebar'>
            <div>
              <button onClick={()=>{loadmodules()}}>See Modules</button>  
              <div className='divPad-small'/>
              <button onClick={()=>{loadmoduletests()}}>See Tests</button>  
            </div>
            <div>
              <h4 >{pagevar["cource_name"]}</h4>
            </div>
            <div>
              <button onClick={()=>{navigate("/addmodule",{state:location.state})}}>Add Module +</button>  
              <div className='divPad-small'/>
              <button onClick={()=>{navigate("/addcoursetest",{state:location.state})}}>Add Tests +</button>  
            </div>
          </Container>
          <div>
            {/* {subcaller(toggle)} */}
